...
public class VelocityCalculatorNoAngles : MonoBehaviour {
    ...
    public void Apply(Rigidbody rigidbody)
    {
        if (rigidbody)
        {
            if (Settings.NumSmoothingFrames > 0)
            {
                ...
                rigidbody.angularVelocity = throwAngularVel * 5;
                rigidbody.velocity = new Vector3(throwVel.x * Settings.ThrowScale.x, throwVel.y * Settings.ThrowScale.y, throwVel.z * Settings.ThrowScale.z) * 2;
            }
            else
            {
                if (Settings.UseAngularVelocity) rigidbody.angularVelocity = _lastAngularVelocity;
                rigidbody.velocity = new Vector3(_lastVelocity.x * Settings.ThrowScale.x, _lastVelocity.y * Settings.ThrowScale.y, _lastVelocity.z * Settings.ThrowScale.z);
            }
        }
    }
